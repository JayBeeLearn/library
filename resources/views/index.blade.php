            ...
            @foreach ($books as $book)
                <div class="flex flex-col justify-between py-2 px-4 bg-green-100 rounded my-4 sm:my-0">
                    ...
                    <div class=" sm:min-h-28 flex justify-center items-center">
                        <a class="text-center text-3xl font-semibold link cursor-pointer" href="{{ route('book.show', $book->id) }}">{{ $book->title }}</a>
                    </div>
                    <div class="">
                        <h3 class="">Written by: <a href="{{ route('author.show', $book->user->id) }}" class="text-center text-xl font-semibold link cursor-pointer" >{{ $book->user->name }}</a></h3>
                    </div>
                </div>
            @endforeach
            </div>
